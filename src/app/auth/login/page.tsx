    ...
    try {
      ...
      if (result?.error) {
        if (result.error === "email-not-verified") {
          ...
          setTimeout(() => router.push(`/auth/verify?email=${encodeURIComponent(email)}`), 3000)
    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative bg-background dark:bg-dark">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="w-full max-w-md relative z-10">
        <div className="bg-white dark:bg-dark-surface rounded-2xl shadow-2xl overflow-hidden border border-gray-100 dark:border-dark-border">
          ...
          <div className="p-8">
            {success && (
              <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} className="mb-4 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg flex items-center gap-2 text-green-700 dark:text-green-400 text-sm">
            ...
            {error && (
              <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} className="mb-4 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg flex items-center gap-2 text-red-700 dark:text-red-400 text-sm">
                <AlertCircle className="w-5 h-5 shrink-0" />
                <div>
                  <p>{error}</p>
                  {error.includes("لم يتم تأكيد") && (
                    <Link href={`/auth/verify?email=${encodeURIComponent(email)}`} className="text-red-800 dark:text-red-300 font-bold hover:underline mt-1 inline-block">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    required
                    className="w-full px-4 py-3 pr-11 border border-gray-300 dark:border-dark-border bg-white dark:bg-dark-card text-gray-900 dark:text-white rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent text-right placeholder:text-gray-400"
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    required
                    className="w-full px-4 py-3 pr-11 pl-11 border border-gray-300 dark:border-dark-border bg-white dark:bg-dark-card text-gray-900 dark:text-white rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent text-right"
                    placeholder="••••••••"
                  />
              ...
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2">
                  <input type="checkbox" className="w-4 h-4 rounded border-gray-300 dark:border-dark-border text-primary focus:ring-primary" />
                  <span className="text-sm text-gray-600 dark:text-gray-400">تذكرني</span>
                </label>
                <Link href="/auth/forgot-password" className="text-sm text-primary dark:text-primary-light hover:underline">
                  نسيت كلمة المرور؟
                </Link>
              </div>
              ...
              <motion.button
                ...
                type="submit"
                disabled={loading}
                className="w-full bg-primary text-white py-3 rounded-lg font-bold hover:bg-primary-light transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {loading ? (
            ...
            <motion.button
              ...
              className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-gray-300 dark:border-dark-border rounded-lg hover:bg-gray-50 dark:hover:bg-dark-card transition-colors dark:text-white"
            >
              ...
              <span className="font-medium">تسجيل الدخول بجوجل</span>
            </motion.button>

            <div className="mt-4 border-t border-gray-100 dark:border-dark-border pt-4">
              <Link href="/ar/graduate/claim" className="w-full flex items-center justify-center gap-2 px-4 py-3 border-2 border-dashed border-primary/30 dark:border-primary/20 text-primary dark:text-primary-light rounded-lg hover:bg-primary/5 dark:hover:bg-primary/10 transition-colors font-medium text-sm">
            ...
            <p className="text-center mt-4 text-sm text-gray-600 dark:text-gray-400">
              ليس لديك حساب؟{" "}
              <Link href="/auth/register" className="text-primary dark:text-primary-light font-bold hover:underline">
                أنشئ حساباً جديداً
              </Link>
            </p>
          </div>
