...
export default function GraduatesImportPage() {
  ...
  const handleFixYears = async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (res.ok) {
        setFixResult(t("graduates.fixResult").replace("{fixed}", String(data.fixed)).replace("{total}", String(data.total)))
        fetchGraduates()
        fetchFilters()
      } else {
        setFixResult(t("graduates.fixError").replace("{msg}", data.error || t("graduates.importFailed")))
      }
    } catch { setFixResult(t("graduates.connError")) }
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.preview) {
        ...
        let errMsg = data.error || t("graduates.importFailed")
        if (data.headers) errMsg += "\n\n" + t("graduates.headersLbl") + ": " + data.headers.join(", ")
        if (data.detected) errMsg += "\n\n" + t("graduates.detectedLbl") + ": " + Object.entries(data.detected).map(([k, v]) => `${k}=${v || "—"}`).join(", ")
        setError(errMsg)
      }
    } catch { setError(t("graduates.connError")) }
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data.error || t("graduates.importFailed"))
      }
    } catch { setError(t("graduates.connError")) }
    finally { setImporting(false) }
  }
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="bg-white dark:bg-[#1a2332] rounded-2xl border border-gray-100 dark:border-[#2a3d56] shadow-sm p-6 mb-8">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            <h2 className="font-semibold text-[#0f2547] dark:text-white">{t("graduates.uploadTitle")}</h2>
            <p className="text-xs text-gray-500 dark:text-gray-400">{t("graduates.uploadHint")}</p>
          </div>
        </div>
        ...
        {!preview && (
          <div className="flex items-center gap-3 flex-wrap">
            ...
            >
              ...
              {importing ? t("graduates.analyzing") : t("graduates.chooseUpload")}
            </button>
            <button
            ...
            >
              ...
              {fixing ? t("graduates.fixing") : t("graduates.fixYears")}
            </button>
          </div>
        )}
          ...
          <div className="mt-4 border border-amber-200 bg-amber-50 dark:bg-amber-500/10 dark:border-amber-500/20 rounded-xl overflow-hidden">
            <div className="p-4 border-b border-amber-200 dark:border-amber-500/20 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium text-amber-800 dark:text-amber-300 text-sm">{t("graduates.previewTitle")}</span>
              </div>
            ...
            <div className="p-4">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm mb-4">
                <div className="bg-white dark:bg-[#1a2332] rounded-lg p-3 text-center"><p className="text-2xl font-bold text-green-600">{preview.validCount}</p><p className="text-xs text-gray-500">{t("graduates.validRows")}</p></div>
                <div className="bg-white dark:bg-[#1a2332] rounded-lg p-3 text-center"><p className="text-2xl font-bold text-amber-600">{preview.skipped}</p><p className="text-xs text-gray-500">{t("graduates.skipped")}</p></div>
                <div className="bg-white dark:bg-[#1a2332] rounded-lg p-3 text-center"><p className="text-2xl font-bold text-gray-600">{preview.total}</p><p className="text-xs text-gray-500">{t("graduates.totalRows")}</p></div>
              </div>
              ...
              {preview.errors?.length > 0 && (
                <details className="mb-4"><summary className="text-xs text-amber-600 cursor-pointer">{t("graduates.errorsCount").replace("{n}", String(preview.errors.length))}</summary>
              ...
              <div className="flex gap-2">
                <button onClick={handleConfirmImport} disabled={importing} className="flex items-center gap-2 px-5 py-2.5 bg-green-600 text-white rounded-lg text-sm font-medium hover:bg-green-700 disabled:opacity-60 transition-colors">
                  ...
                  {t("graduates.confirmImport").replace("{n}", String(preview.validCount))}
                </button>
                <button onClick={handleCancelPreview} className="px-4 py-2.5 border border-gray-300 rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition-colors">{t("common.cancel")}</button>
              </div>
            </div>
          </div>
        )}
        ...
        {result && (
          <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-xl">
            <div className="flex items-center gap-2 text-green-700 font-medium mb-2"><CheckCircle className="w-5 h-5" />{t("graduates.successTitle")}</div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
              <div className="bg-white rounded-lg p-3 text-center"><p className="text-2xl font-bold text-[#0f2547]">{result.imported}</p><p className="text-xs text-gray-500">{t("graduates.imported")}</p></div>
              <div className="bg-white rounded-lg p-3 text-center"><p className="text-2xl font-bold text-amber-600">{result.skipped}</p><p className="text-xs text-gray-500">{t("graduates.ignored")}</p></div>
              <div className="bg-white rounded-lg p-3 text-center"><p className="text-2xl font-bold text-gray-600">{result.total}</p><p className="text-xs text-gray-500">{t("graduates.totalRows")}</p></div>
            </div>
            {result.errors?.length > 0 && (
              <details className="mt-3"><summary className="text-xs text-amber-600 cursor-pointer">{t("graduates.showErrors").replace("{n}", String(result.errors.length))}</summary>
        ...
        <div className="p-4 border-b border-gray-100 dark:border-[#2a3d56]">
          <div className="flex gap-3 mb-3">
            ...
            <button onClick={() => setShowFilters(!showFilters)} className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium transition-colors ${showFilters ? "bg-primary text-white" : "bg-gray-100 dark:bg-[#1e2d42] text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-[#253347]"}`}>
              <Filter className="w-4 h-4" />{t("graduates.filterBtn")}
            </button>
          </div>
          ...
          {showFilters && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div>
                <label className="text-xs text-gray-500 mb-1 block">{t("members.faculty")}</label>
              ...
              <div>
                <label className="text-xs text-gray-500 mb-1 block">{t("members.country")}</label>
              ...
              <div>
                <label className="text-xs text-gray-500 mb-1 block">{t("members.graduationYear")}</label>
              ...
              <div>
                <label className="text-xs text-gray-500 mb-1 block">{t("members.specialization")}</label>
          ...
          {showFilters && (filterFaculty || filterCountry || filterYear || filterSpec) && (
            <div className="mt-3 flex items-center gap-2 flex-wrap">
              <span className="text-xs text-gray-400">{t("graduates.activeFilters")}</span>
              ...
              <button onClick={() => { setFilterFaculty(""); setFilterCountry(""); setFilterYear(""); setFilterSpec(""); fetchGraduates(search) }} className="text-xs text-red-500 hover:text-red-700">{t("graduates.clearAll")}</button>
            </div>
          )}
        </div>

        <div className="px-4 py-2 border-b border-gray-50 dark:border-[#253347] text-xs text-gray-400">
          {totalCount > 0 && <span>{t("graduates.totalResults")} <strong className="text-gray-600 dark:text-gray-300">{totalCount}</strong></span>}
        </div>

        {loadingGrads ? (
          <div className="p-8 text-center text-gray-400"><Loader2 className="w-8 h-8 mx-auto animate-spin" /></div>
        ) : graduates.length === 0 ? (
          <div className="p-8 text-center text-gray-400"><FileSpreadsheet className="w-12 h-12 mx-auto mb-2 opacity-40" /><p>{t("graduates.noGraduates")}</p></div>
        ) : (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>{graduates.map((g: any) => (
                <tr key={g.id} className="border-b border-gray-50 dark:border-[#253347] hover:bg-gray-50/50 dark:hover:bg-[#1e2d42]/50">
                  ...
                  <td className="px-4 py-3">{g.isClaimed ? <span className="text-green-600 text-xs">{t("graduates.claimed")}</span> : <span className="text-amber-600 text-xs">{t("graduates.notClaimed")}</span>}</td>
                </tr>
              ))}</tbody>
            </table>
          </div>
