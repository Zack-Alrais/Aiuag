...
export default function NotificationsPage() {
  ...
  const getTypeLabel = (type: string) => {
    switch (type) {
      case "success": return t("notifications.type.success")
      case "warning": return t("notifications.type.warning")
      case "error": return t("notifications.type.error")
      default: return t("notifications.type.info")
    }
  }

  const getEntityLabel = (entityType: string | null) => {
    switch (entityType) {
      case "member": return t("notifications.entity.member")
      case "news": return t("notifications.entity.news")
      case "event": return t("notifications.entity.event")
      case "contact": return t("notifications.entity.contact")
      case "donation": return t("notifications.entity.donation")
      case "newsletter": return t("notifications.entity.newsletter")
      case "gallery": return t("notifications.entity.gallery")
      default: return ""
    }
  }
    ...
    <div className="space-y-6 dark:bg-[#0b1120] min-h-screen p-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-800 dark:text-[#f1f5f9]">{t("notifications.title")}</h1>
...
{t("contacts.all")}
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-[#1a2332] rounded-xl shadow-sm dark:shadow-[0_2px_8px_rgba(0,0,0,0.4)] border border-gray-100 dark:border-[#2a3d56] p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-600 dark:text-[#cbd5e1] mb-2">{t("notifications.emptyTitle")}</h3>
          <p className="text-sm text-gray-500 dark:text-[#94a3b8]">
            {filter === "unread" ? t("notifications.allRead") : t("notifications.emptySub")}
          </p>
        </div>
      ) : (
              ...
              <div key={notif.id} className={`flex items-start gap-4 p-4 hover:bg-gray-50 dark:hover:bg-[#1e2d42] transition-colors ${!notif.isRead ? "bg-blue-50/30 dark:bg-[#0d1a30]" : ""}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {!notif.isRead && (
                      <span className="px-1.5 py-0.5 bg-blue-500 text-white text-[10px] font-bold rounded">{t("notifications.new")}</span>
                    )}
                  ...
                  <p className="text-sm text-gray-600 dark:text-[#cbd5e1] mb-1">{lang === "ar" ? notif.messageAr : notif.messageEn}</p>
                  <p className="text-xs text-gray-400 dark:text-[#7a8ba3]">{new Date(notif.createdAt).toLocaleDateString(lang === "ar" ? "ar-EG" : "en-US", { year: "numeric", month: "long", day: "numeric", hour: "2-digit", minute: "2-digit" })}</p>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  {!notif.isRead && (
                    <button onClick={() => markAsRead(notif.id)} className="p-2 hover:bg-gray-200 dark:hover:bg-[#2a3d56] rounded-lg text-gray-400 dark:text-[#94a3b8] hover:text-blue-600 dark:hover:text-[#60a5fa] transition-colors" title={t("notifications.markReadTitle")}>
                      <Eye className="w-4 h-4" />
                    </button>
                  )}
                  <button onClick={() => deleteNotification(notif.id)} className="p-2 hover:bg-gray-200 dark:hover:bg-[#2a3d56] rounded-lg text-gray-400 dark:text-[#94a3b8] hover:text-red-600 dark:hover:text-[#f87171] transition-colors" title={t("notifications.deleteTitle")}>
