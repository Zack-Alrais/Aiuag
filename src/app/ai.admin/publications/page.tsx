    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-800 dark:text-[#f1f5f9]">{t("publications.title")}</h1>
            <p className="text-sm text-gray-500 dark:text-[#94a3b8]">{t("publications.count").replace("{n}", String(items.length))}</p>
          </div>
        </div>
        <button onClick={openAddModal} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 text-sm font-medium">
          <Plus className="w-4 h-4" /> {t("publications.addBtn")}
        </button>
      </div>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-[#253347]">
                {items.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50/50 dark:hover:bg-[#1e2d42]">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500 dark:text-[#94a3b8]">{new Date(item.createdAt).toLocaleDateString(lang === "ar" ? "ar-EG" : "en-GB")}</td>
                    <td className="px-6 py-4"><div className="flex items-center justify-end gap-1">
                      ...
                      {deleteConfirmId === item.id ? (<div className="flex items-center gap-1"><button onClick={() => handleDelete(item.id)} className="px-2.5 py-1 bg-red-600 text-white text-xs rounded-md">{t("common.confirm")}</button><button onClick={() => setDeleteConfirmId(null)} className="px-2.5 py-1 bg-gray-200 dark:bg-[#2a3d56] text-gray-600 dark:text-[#cbd5e1] text-xs rounded-md">{t("common.cancel")}</button></div>) : (<button onClick={() => setDeleteConfirmId(item.id)} className="p-2 text-red-500 dark:text-[#f87171] hover:bg-red-50 dark:hover:bg-[#1e2d42] rounded-lg"><Trash2 className="h-4 w-4" /></button>)}
                    </div></td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-[#1a2332] rounded-2xl shadow-2xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-[#253347]">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-[#f1f5f9]">{editingItem ? t("publications.editTitle") : t("publications.addTitle")}</h2>
            ...
            <form onSubmit={handleSubmit} className="px-6 py-5 space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-gray-700 dark:text-[#94a3b8] mb-1">{t("publications.titleArLabel")} *</label><input type="text" required dir="rtl" value={form.title} onChange={(e) => handleFieldChange("title", e.target.value)} className="w-full px-3 py-2 border border-gray-300 dark:bg-[#111927] dark:border-[#3b4f6b] dark:text-[#f1f5f9] rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder={t("publications.titleArPh")} /></div>
                <div><label className="block text-sm font-medium text-gray-700 dark:text-[#94a3b8] mb-1">{t("publications.titleEnLabel")} *</label><input type="text" required value={form.titleEn} onChange={(e) => handleFieldChange("titleEn", e.target.value)} className="w-full px-3 py-2 border border-gray-300 dark:bg-[#111927] dark:border-[#3b4f6b] dark:text-[#f1f5f9] rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder={t("publications.titleEnPh")} /></div>
              </div>
              <div><label className="block text-sm font-medium text-gray-700 dark:text-[#94a3b8] mb-1">{t("common.description")}</label><textarea rows={3} dir="rtl" value={form.description} onChange={(e) => handleFieldChange("description", e.target.value)} className="w-full px-3 py-2 border border-gray-300 dark:bg-[#111927] dark:border-[#3b4f6b] dark:text-[#f1f5f9] rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none resize-none" placeholder={t("publications.descPh")} /></div>
              <div><label className="block text-sm font-medium text-gray-700 dark:text-[#94a3b8] mb-1">{t("common.category")}</label><input type="text" value={form.category} onChange={(e) => handleFieldChange("category", e.target.value)} className="w-full px-3 py-2 border border-gray-300 dark:bg-[#111927] dark:border-[#3b4f6b] dark:text-[#f1f5f9] rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder={t("publications.categoryPh")} /></div>
              <div><label className="block text-sm font-medium text-gray-700 dark:text-[#94a3b8] mb-1"><span className="flex items-center gap-1.5"><FileText className="h-3.5 w-3.5" /> {t("publications.fileLabel")}</span></label><FileUpload value={form.fileUrl} onChange={(url) => handleFieldChange("fileUrl", url)} folder="publications" type="pdf" /></div>
              <div><label className="block text-sm font-medium text-gray-700 dark:text-[#94a3b8] mb-1">{t("publications.coverLabel")}</label><FileUpload value={form.imageUrl} onChange={(url) => handleFieldChange("imageUrl", url)} folder="publications/covers" type="image" /></div>
              <div className="flex items-center justify-end gap-3 pt-3 border-t border-gray-100 dark:border-[#253347]">
                <button type="button" onClick={closeModal} className="px-4 py-2 text-sm font-medium text-gray-600 dark:text-[#cbd5e1] bg-gray-100 dark:bg-[#2a3d56] hover:bg-gray-200 dark:hover:bg-[#3b4f6b] rounded-lg">{t("common.cancel")}</button>
                <button type="submit" disabled={submitting} className="px-5 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg disabled:opacity-50">{submitting ? t("common.saving") : editingItem ? t("publications.updateBtn") : t("publications.createBtn")}</button>
              </div>
            </form>
          </div>
        </div>
