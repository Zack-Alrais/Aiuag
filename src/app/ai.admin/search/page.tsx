      ...
      try {
        ...
        if (membersRes.ok) {
          ...
          members.slice(0, 5).forEach((m: any) => {
            allResults.push({
              type: "member",
              title: m.user?.name || m.name || t("search.type.member"),
        ...
        if (newsRes.ok) {
          ...
          news.slice(0, 5).forEach((n: any) => {
            allResults.push({
              type: "news",
              title: n.titleAr || n.titleEn || t("search.type.news"),
              description: n.status === "published" ? t("search.news.published") : t("search.news.draft"),
              href: "/ai.admin/news",
            })
          })
        }
        ...
        if (eventsRes.ok) {
          ...
          events.slice(0, 5).forEach((e: any) => {
            allResults.push({
              type: "event",
              title: e.titleAr || e.titleEn || t("search.type.event"),
        ...
        if (projectsRes.ok) {
          ...
          projects.slice(0, 5).forEach((p: any) => {
            allResults.push({
              type: "project",
              title: p.titleAr || p.titleEn || t("search.type.project"),
  ...
  return (
    <div className="space-y-6 dark:bg-[#0b1120] min-h-screen p-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 dark:text-[#f1f5f9]">{t("search.title")}</h1>
          {query && (
            <p className="text-gray-500 dark:text-[#94a3b8] text-sm mt-1">
              {t("search.resultsFor").split("{q}").map((part, i, arr) => (
                <Fragment key={i}>
                  {part}
                  {i < arr.length - 1 && <span className="font-medium text-[#1A3A6B] dark:text-[#60a5fa]">{query}</span>}
                </Fragment>
              ))}
            </p>
          )}
      ...
      {loading ? (
        <div className="flex items-center justify-center h-64">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-gray-500 dark:text-[#94a3b8]">{t("search.loading")}</p>
          </div>
        </div>
      ) : results.length === 0 ? (
        <div className="bg-white dark:bg-[#1a2332] rounded-2xl shadow-sm dark:shadow-[0_2px_8px_rgba(0,0,0,0.4)] border border-gray-100 dark:border-[#2a3d56] p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-600 dark:text-[#cbd5e1] mb-2">
            {query ? t("search.emptyTitle") : t("search.emptyTitlePrompt")}
          </h3>
          <p className="text-sm text-gray-500 dark:text-[#94a3b8]">
            {query
              ? t("search.emptySub").replace("{q}", query)
              : t("search.emptySubPrompt")}
          </p>
        </div>
      ) : (
